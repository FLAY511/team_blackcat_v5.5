#!/usr/bin/env bash
# CYBER FLAY v5.5 (SAFE) — Full features package (OSINT optimal, username check, phone lookup, JSO helper, utilities)
...
web_member_info(){
  ...
  # Cari baris yang berisi kata kunci terkait daftar/anggota/nama
  grep -E -i '(^| )(nama|anggota|member|user|account|profil|profile|daftar|list|participant|anggota|nama anggota|nama:)' "${tmp}.lines" | sed 's/^[ \\t]*//;s/[ \\t]*$//' | uniq > "${tmp}.found"

  # Jika tidak ada hasil dari kata kunci, coba ekstraksi pola nama (dua kata kapital)
  if [[ ! -s "${tmp}.found" ]]; then
    awk 'length($0)>2' "${tmp}.lines" | grep -E '([A-Z][a-z]+ [A-Z][a-z]+)' | sed 's/^[ \\t]*//;s/[ \\t]*$//' | uniq > "${tmp}.found"
  fi
